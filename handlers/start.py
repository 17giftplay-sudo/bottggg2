...
async def cmd_start(message: Message, state: FSMContext, bot: Bot, command: CommandObject = None):
    try:
        ...
        if referrer_id and referrer_id != message.from_user.id:
            from database import set_user_pending_referrer
            await set_user_pending_referrer(message.from_user.id, referrer_id)
            user = await get_user(message.from_user.id)
        ...
        target_ref = referrer_id or user_referred_by

        if target_ref and target_ref == message.from_user.id:
            await message.answer(
        ...
        elif target_ref and target_ref != message.from_user.id and not dev_verified:
...
async def check_join(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if joined:
        ...
        if pending_ref and not user_referred_by and not dev_verified:
            ...
            if require_fp:
                ...
                await callback.message.edit_text(verify_prompt, reply_markup=builder.as_markup(), parse_mode="HTML")
                await callback.answer()
                return
